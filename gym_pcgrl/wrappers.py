...
class ToImage(gym.Wrapper):
    def __init__(self, game, names, **kwargs):
        ...
        for n in names:
            assert n in self.env.observation_space.spaces.keys(
        ...
        for n in self.names:
            if len(final.shape) == 0:
                ...
                final = np.append(final,
                                  obs[n].reshape(self.shape[0], self.shape[1],
                                                 -1),
                                  axis=2)
...
class OneHotEncoding(gym.Wrapper):
    ...
    def __init__(self, game, name, **kwargs):
        ...
        assert name in self.env.observation_space.spaces.keys(
        ...
        self.dim = self.observation_space.spaces[self.name].high.max(
        ) - self.observation_space.spaces[self.name].low.min() + 1
...
class ActionMap(gym.Wrapper):
    def __init__(self, game, **kwargs):
        ...
        assert 'map' in self.env.observation_space.spaces.keys(
...
class Cropped(gym.Wrapper):
    def __init__(self, game, crop_size, pad_value, name, **kwargs):
        ...
        assert 'pos' in self.env.observation_space.spaces.keys(
        ), 'This wrapper only works for representations thave have a position'
        assert name in self.env.observation_space.spaces.keys(
        ), 'This wrapper only works if you have a {} key'.format(name)
        assert len(self.env.observation_space.spaces[name].shape
                   ) == 2, "This wrapper only works on 2D arrays."
        self.name = name
        self.size = crop_size
...
class CroppedImagePCGRLWrapper(gym.Wrapper):
    def __init__(self, game, crop_size, **kwargs):
        ...
        env = Cropped(self.pcgrl_env, crop_size,
                      self.pcgrl_env.get_border_tile(), 'map')
...
class ActionMapImagePCGRLWrapper(gym.Wrapper):
    ...
    def adjust_param(self, **kwargs):
        return self.pcgrl_env.adjust_param(**kwargs)
